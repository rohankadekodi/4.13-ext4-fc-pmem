...
{
	ext4_inode_cachep = kmem_cache_create("orig_ext4_fc_inode_cache",
...
static int ext4_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	ext4_group_t first_not_zeroed;
	ext4_fsblk_t fc_pblock = 0;
	sector_t fc_sector = 0;
	pgoff_t fc_pgoff;
	size_t fc_size, fc_map_len;
	void *fc_kaddr;
	int fc_ret;
	pfn_t fc_pfn_t;
	struct dax_device *fc_dax_dev;
	
	fc_size = EXT4_NUM_FC_BLKS * PAGE_SIZE;
	...
	jbd2_journal_bmap(sbi->s_journal, sbi->s_journal->j_first_fc, &fc_pblock);

	fc_sector = fc_pblock << 3;
	fc_ret = bdev_dax_pgoff(sb->s_bdev, fc_sector, fc_size, &fc_pgoff);
	if (fc_ret) {
		BUG();
	}

	fc_dax_dev = fs_dax_get_by_host(sb->s_bdev->bd_disk->disk_name);
	if (!fc_dax_dev) {
		BUG();
	}
	
	fc_map_len = dax_direct_access(fc_dax_dev, fc_pgoff, PHYS_PFN(fc_size),
				    &fc_kaddr, &fc_pfn_t);
	if (fc_map_len < 0) {
		BUG();
	}

	sbi->fc_journal_start = (unsigned long) fc_kaddr;
